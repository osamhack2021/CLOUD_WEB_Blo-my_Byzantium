const {firearmQuery, modifyFirearmTransaction} = require('./firearmChaincode.js');
const {foodQuery, modifyFirearmTransaction} = require('./foodChaincode.js');
...
router.route('/query/:serialNumber').get((req,res)=>{
    ...
    firearmQuery(serialNumber).then((value)=>{ // 하이퍼레저에서 query에 대한 응답을 가져오는 부분
...
router.route('/createFirearm/:serialNumber/:model/:owner/:affiliatedUnit/:status/:updateReason').get((req,res)=>{
    ...
    modifyFirearmTransaction(['createFirearm',serialNumber,model,owner,affiliatedUnit,status,updateReason]).then((value)=>{
        res.writeHead(200,{"Content-Type":'text/html;charset=utf8'});
        res.write(value);
        res.end();
    });
...
router.route('/checkoutFirearm/:serialNumber/:status/:updateReason').get((req,res)=>{
    ...
    console.log(`status = ${status}`);
    console.log(`updateReason = ${updateReason}`);
    modifyFirearmTransaction(['checkoutFirearm',serialNumber,status,updateReason]).then((value)=>{
        res.writeHead(200,{"Content-Type":'text/html;charset=utf8'});
        res.write(value);
        res.end();
    })
...
router.route('/checkinFirearm/:serialNumber/:status/:updateReason').get((req,res)=>{
    ...
    console.log(`status = ${status}`);
    console.log(`updateReason = ${updateReason}`);
    modifyFirearmTransaction(['checkinFirearm',serialNumber,status,updateReason]).then((value)=>{
        res.writeHead(200,{"Content-Type":'text/html;charset=utf8'});
        res.write(value);
        res.end();
    });
...
router.route('/changeFirearmAttributes/:serialNumber/:model/:owner/:affiliatedUnit/:status/:updateReason').get((req,res)=>{
    ...
    console.log(`status = ${status}`);
    console.log(`updateReason = ${updateReason}`);
    modifyFirearmTransaction(['changeFirearmAttributes',serialNumber,model,owner,affiliatedUnit,status,updateReason]).then((value)=>{
        res.writeHead(200,{"Content-Type":'text/html;charset=utf8'});
        res.write(value);
        res.end()
    });
...
router.route('/deleteFirearm/:serialNumber').get((req,res)=>{
    ...
    console.log('-------총기 삭제---------')
    console.log(`serialNumber = ${serialNumber}`);
    modifyFirearmTransaction(['deleteFirearm',serialNumber]).then((value)=>{
        res.writeHead(200,{"Content-Type":'text/html;charset=utf8'});
        res.write(value);
        res.end()
    });
